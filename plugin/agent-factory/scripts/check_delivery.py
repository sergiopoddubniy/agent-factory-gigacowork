...
#
# Повод: в поставке Клиент-П карточка требовала активировать «Разбор тендерной
...
def _склеить_кавычки(строки: list[str]) -> list[str]:
    ...
    Длинные имена агентов Клиент-А не помещаются в строку, и кавычка
...
def вызовы_активации(блок: str) -> tuple[list[str], int | None]:
    ...
    Клиент-А активация написана прозой, а ниже в том же поле идёт нумерованный
...
def шаги_активации(блок: str) -> tuple[list[str], bool]:
    ...
    данными ДатаРеестр“»), и по ней вызовы считать нельзя: получилось бы, что
    ...
    действия активируй по порядку:». Без этого признака на комплекте Клиент-М
    вышло «пунктов 5, а сказано три», а на карточке Клиент-А шагами
...
def зовомые_навыки(строки) -> list[str]:
    ...
    навык «ничего не найдено» — красное на исправном комплекте Клиент-А.
    """
    имена: list[str] = []
    for ln in строки:
...
def _имя_из_строки(ln: str) -> str | None:
    ...
    Две живые конвенции: Клиент-П и Клиент-М берут имя в кавычки, Клиент-А пишет его
    голым текстом — «Активируй навык Работа с данными Клиент-А». Читатель,
    знающий только кавычки, на карточках Клиент-А не находил ничего и
...
def правило_отказа(блок: str, имена: list[str]) -> tuple[bool, list[str]]:
    ...
    косвенном падеже — «без „Работы с данными ДатаРеестр“». Дословное сравнение
...
def block_graph(root: Path, files, dirs) -> None:
    ...
    # комплекте все девять агентов Клиент-А выглядели недостижимыми только
...
def block_relations(root: Path, files, dirs) -> None:
    ...
    for a in агенты:
        ...
        #
        # Повод: у всех восьми агентов Клиент-П раздел «Рекомендуемые навыки»
        ...
        #
        # Повод: у Клиент-П файл команд агента 8 говорил «8 команд», а заголовков
        ...
        #
        # Карта бывает и таблицей с номерами (Клиент-А), и прозой (Клиент-П), а
...
def core(root: Path, files, dirs, gen: str, calc: bool, task: str) -> None:
    ...
    # Архив опознаётся по СОДЕРЖИМОМУ, а не по расширению: в поставке Клиент-П
    ...
    #
    # Повод: в папке коннектора Клиент-П лежал `login and password _ for test_
    ...
    if kbs:
        ...
        # существует»), и этого мало: карта отстаёт от папки молча. У Клиент-П
        # так вышло дважды — `критерии_отбора_Клиент-П.md` с порогами, названными
    ...
    #
    # Повод: навык «Работа с данными ДатаРеестр» лежал в поставке, файлы базы
    ...
    if space_skills:
        ...
        #
        # Повод: редакция навыка ДатаРеестр от 25.08 назвала три инструмента,
        ...
        if tool_named:
            ...
            # версии: у Клиент-А коннектор версионируется датой контракта
            ...
            #
            # Повод: инструкция Клиент-П обещала «4.0.2 и новее», а навык в шапке
    ...
    #
    # Повод: в сервере ДатаРеестр готовность заказа определялась поиском слов
    ...
    for f in agent_skills:
        ...
        #
        # Повод: боевой прогон Клиент-А 28.08.2026. Карточка активировала только
        ...
        if назван_коннектор:
            ...
            # **«Работа с данными ДатаРеестр»**» — то же самое, что без звёздочек.
        ...
        # ложных красных строк на портфеле Клиент-М: у агента консолидации это
    ...
    # правки: 27.08.2026 правка агента 1 Клиент-П ушла в папку пакета и не ушла в
    ...
    if pkg_root is not None:
        ...
        #
        # Повод: у Клиент-П портфель вырос с семи агентов до восьми, а фразы
    ...
    #
    # Повод: правка навыка агента рыночной разведки Клиент-П (Федресурс, остановка
    ...
    # Клиент-П шесть агентов из семи совпадали байт в байт ровно до того часа,
    ...
    for old, new, src in dict.fromkeys(
        ...
        for f in files:
            ...
            for line in t.split("\n"):
                ...
                # все файлы подряд, в том числе `07_Коннектор_ДатаРеестр/Протокол
...
def приёмочные_наборы(files, dirs):
    ...
    по существу: у Клиент-П так и было — кейсы существовали только у агента № 8.
    """
    pkg_root = None
    for d in dirs:
        if "пакеты агентов" in nameflat(d.name):
...
# ───────────────────── уровень агентизации ─────────────────────
#
# Повод — Клиент-А, 15.09.2026. Портфель был собран по джобам («провести
